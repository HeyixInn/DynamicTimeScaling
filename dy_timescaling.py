...
if __name__=="__main__":
    ...
    prompts_no_budget = [get_prompt(q, model_type, tok, enable_thinking=True) for q in questions]
...
# ---------------------------------------------------------------------------
    ...
    for i in range(0, len(prompts_thinking), len(dataset)):
        if args.data_name =="aime":
            ...
            eval_results = llm_evaluate(outputs[i:i+len(dataset)], solutions*len(SEEDs), questions*len(SEEDs))
    ...
    for iter in range(args.max_iter):
        ...
        pairs = gp.get_guidance_pairs(len(gp.guide_pool))
        ...
        if args.data_name =="aime":
            ...
            eval_outputs = llm_evaluate(outputs, [solutions[idx] for idx in sampled_idx], [questions[idx] for idx in sampled_idx])
        ...
        if args.data_name =="aime":
            ...
            eval_new_guide = llm_evaluate(outputs, solutions, questions)
